...
data GlobalVariables = GlobalVariables
  { saveState :: (Int, Int),
    ...
    playbackKeys :: [[Key]],
    playbackSaveState :: (Int, Int),
    playBackName :: Maybe String,
    recordSaveState :: (Int, Int),
    saveHiScore :: Int
  }
...
main =
  do
    ...
    (recMode, keys, rss, repName) <-
      if isJust $ getReplayFilename args
        then do
          ...
          if "-r" `elem` args
            then return (Play, [], (1, 0), Nothing)
            else return (Record, [], (1, 0), Nothing)
    ...
    mainLoop
    destroyWindow curwnd `catch` (\(SomeException _) -> return ())
  where
...
initMatrix = do
  ...
  lookAt (Vertex3 @GLdouble 0 0 927) (Vertex3 @GLdouble 0 0 0) (Vector3 @GLdouble 0 1 0)
...
openingProc clock menuCursor vars ks =
      ...
      if clock > demoStartTime
        then demoStart vars
          ...
          if clock < drawCompleteTime
            then color $ Color3 @GLdouble 0 0.2 0.8
            else color $ Color3 @GLdouble (0 + shine clock) (0.2 + shine clock) (0.8 + shine clock)

          preservingMatrix $ do
            translate (Vector3 @GLdouble 0 120 0)
            scale @GLdouble 1.05 1 1
          ...
          preservingMatrix $ do
            translate $ Vector3 @GLdouble (-195) 130 0
            scale @GLdouble 0.73 0.56 0.56
            renderStringGrad Roman 0 "Monadius"

          preservingMatrix $ do
            if menuCursor == 0
              then color $ Color3 @GLdouble 1.0 1.0 0
              else color $ Color3 @GLdouble 1.0 1.0 1.0
            translate $ Vector3 @GLdouble (-230) (-200) 0
            scale @GLdouble 0.2 0.2 0.3
          ...
          preservingMatrix $ do
            ...
            translate $ Vector3 @GLdouble 70 (-200) 0
            scale @GLdouble 0.2 0.2 0.3
          ...
          preservingMatrix $ do
            translate $ Vector3 @GLdouble (-250) 75 0
            scale @GLdouble 0.15 0.10 0.15
            renderStringGrad Roman 10 "Dedicated to the makers, the players, the history,"

          preservingMatrix $ do
            translate $ Vector3 @GLdouble (-250) 55 0
            scale @GLdouble 0.15 0.10 0.15
          ...
          mapM_
            ( \(y, (strA, strB), i) ->
                preservingMatrix $ do
                  preservingMatrix $ do
                    translate $ Vector3 @GLdouble (-180) y 0
                    scale @GLdouble 0.18 0.18 0.2
                    renderStringGrad Roman (20 + i * 5) strA

                  preservingMatrix $ do
                    translate $ Vector3 @GLdouble 60 y 0
                    scale @GLdouble 0.18 0.18 0.2
          ...
          if Char ' ' `elem` keystate && clock >= timeLimit
            then
              if menuCursor == 0
                then gameStart 1 0 False (recorderMode vars) vars
                else gameStart savedLevel savedArea (isCheat vars) (recorderMode vars) vars
            else
              if isJust $ getNumberKey keystate
                then gameStart (fromJust $ getNumberKey keystate) 0 True (recorderMode vars) vars
  ...
  where
    ...
    getNumberKey = foldl mplus Nothing . map keyToNumber
          ...
          initialRecorder
            ...
            ( initialMonadius
                GameVariables
                  { totalScore = 0,
                    flagGameover = False,
                    hiScore = saveHiScore vrs,
                    nextTag = 0,
                    gameClock = savePoints !! area,
                    baseGameLevel = level,
    ...
    moreVertices (a : b : cs) =
      if magnitude (a - b) > d
        then moreVertices (a : ((a + b) / (2 :+ 0)) : b : cs)
        else a : moreVertices (b : cs)
      where
        d = 6
    moreVertices x = x
    ...
    renderVertices2D = mapM_ (\(x :+ y) -> vertex $ Vertex3 x y 0)
...
endingProc vars ks ctr = do
  ...
  zipWithM_
    ( \str pos -> preservingMatrix $ do
        translate $ Vector3 @GLdouble (-180) (-240 + counter - pos) 0
        scale @GLdouble 0.3 0.3 0.3
  ...
  if Char ' ' `elem` keystate
    then return $ Scene $ openingProc 0 1 vars ks
...
mainProc vars gs ks = do
  ...
  if isGameover gamestate
    then do
      counter <- newIORef @GLdouble 0.0
      if mode gamestate /= Record
        then return ()
        else writeReplay vars gamestate $ show $ ReplayInfo (recordSaveState vars, (encode2 . preEncodedKeyBuf) gamestate)

      if currentLevel > 1 && (not . isCheat) vars && (mode gamestate /= Playback)
        then return $ Scene $ endingProc vars {saveState = currentSave, saveHiScore = currentHi} ks counter
  ...
  where
    ...
    searchForNewFile prefix i = do
      ...
      if not b
        then return fn
        else searchForNewFile prefix $ i + 1
    uniqStrs = "" : map (("." ++) . show @Int) [1 ..]
